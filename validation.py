...
def log_symptoms(user_id, date_str, symptoms_list):
    ...
    if BLEEDING_SYMPTOMS & symptom_set:
        ...
        if profile not in PAUSED_PROFILES:
            ...
            if run_length >= 2:
                # confirmed by a second consecutive bleeding day (light or
                # heavy, in any combination) - sync the whole run
                cycle_result = check_and_merge_cycle(user_id, run_start.isoformat(), run_length)
            elif near_existing_period:
                # an isolated single day, but close enough to an existing
                # period to plausibly be its tail end - still extends it
                cycle_result = check_and_merge_cycle(user_id, date_str, 1)
            # else: an isolated single day, far from any existing period -
            # not enough evidence yet that a new period has started, so
            # it's recorded as a symptom only (this now applies to heavy
            # bleeding too, not just light)
